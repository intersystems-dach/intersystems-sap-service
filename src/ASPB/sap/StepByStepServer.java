...
public class StepByStepServer {
     ...
     * function is pretty simple - it has 1 input parameter and 2 output parameter.
     * The content
     * of the input parameter REQUTEXT is copied to the output parameter ECHOTEXT.
     * The
     ...
     * JCoServerFactory. The requested instance
     * will be created, or an existing one will be returned if the instance was
     * created before. It is not possible to
     * run more then one instance with a particular configuration. Then we register
     * the implementation for the
     * function STFC_CONNECTION provided by class StfcConnectionHandler through
     * FunctionHandlerFactory
     * provided by JCo. You are free to write your own implementation
     * JCoServerFunctionHandlerFactory, if you need more
     * than simple mapping between function name and java class implementing the
     * function.
     * Now we can start the server instance. After a while the JCo runtime opens the
     * server connections. You may
     ...
     * At first we write am implementation for JCoServerTIDHandler interface. This
     * implementation is
     * registered by the server instance and will be used for each call send in
     * "background task". Without
     * such implementation JCo runtime deny any tRFC calls. See javadoc for
     * interface JCoServerTIDHandler for
     * details.
     */
     ...
     * available only on Java side. At first
     * we create the respective function meta data, because the function is not
     * available in ABAP DDIC. Then the function
     * meta data is stored in a custom repository which is registered with the
     * server instance. Naturally we also
